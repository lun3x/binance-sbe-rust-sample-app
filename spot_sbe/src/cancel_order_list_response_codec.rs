...
pub use decoder::CancelOrderListResponseDecoder;
pub use encoder::CancelOrderListResponseEncoder;

pub const SBE_BLOCK_LENGTH: u16 = 21;
pub const SBE_TEMPLATE_ID: u16 = 312;
...
pub mod encoder {
    ...
    pub struct CancelOrderListResponseEncoder<'a> {
        buf: WriteBuf<'a>,
    ...
    impl<'a> Encoder<'a> for CancelOrderListResponseEncoder<'a> {
        #[inline]
    ...
    impl<'a> CancelOrderListResponseEncoder<'a> {
...
pub mod decoder {
    ...
    pub struct CancelOrderListResponseDecoder<'a> {
        buf: ReadBuf<'a>,
        initial_offset: usize,
        offset: usize,
        limit: usize,
    ...
    impl<'a> Reader<'a> for CancelOrderListResponseDecoder<'a> {
        #[inline]
    ...
    impl<'a> CancelOrderListResponseDecoder<'a> {
        pub fn wrap(
            mut self,
            buf: ReadBuf<'a>,
            offset: usize,
        ...
        P: Decoder<'a> + Default,
    {
        ...
        /// group token - Token{signal=BEGIN_GROUP, name='orderReports', referencedName='null', description='null', packageName='null', id=101, version=0, deprecated=0, encodedLength=124, offset=-1, componentTokenCount=134, encoding=Encoding{presence=REQUIRED, primitiveType=null, byteOrder=LITTLE_ENDIAN, minValue=null, maxValue=null, nullValue=null, constValue=null, characterEncoding='null', epoch='null', timeUnit=null, semanticType='null'}}
        #[inline]
        pub fn parent(&mut self) -> SbeResult<P> {
            self.parent.take().ok_or(SbeErr::ParentNotSet)
        }
        ...
        pub fn strategy_type(&self) -> Option<i32> {
            let value = self.get_buf().get_i32_at(self.offset + 100);
        ...
        pub fn order_capacity(&self) -> OrderCapacity {
            self.get_buf().get_u8_at(self.offset + 104).into()
        }

        /// REQUIRED enum
        #[inline]
        pub fn working_floor(&self) -> Floor {
            self.get_buf().get_u8_at(self.offset + 105).into()
        }
        ...
        pub fn prevented_quantity(&self) -> Option<i64> {
            let value = self.get_buf().get_i64_at(self.offset + 107);
            if value == -9223372036854775808_i64 {
                None
            } else {
                Some(value)
        ...
        pub fn orig_quote_order_qty(&self) -> i64 {
            ...
            self.get_buf().get_i64_at(self.offset + 116)
        }
